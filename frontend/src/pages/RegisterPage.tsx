...
export default function RegisterPage() {
  const [form, setForm]       = useState<RegisterPayload>({
    name: '', phone: '', flatNumber: '', tower: '', password: '',
  })
  ...
  const set = (k: keyof RegisterPayload) =>
  ...
  if (done) return (
    <div className="min-h-[100dvh] w-[80%] flex items-center justify-center bg-[var(--bg)] px-5 py-6">
      <div className="text-center">
        <div className="text-6xl mb-4">✅</div>
        <h2 className="font-['Open Sans'] text-[28px] font-extrabold text-[var(--text)] mb-1">Request submitted!</h2>
  ...
  return (
    <div className="min-h-[100dvh] flex items-center justify-center bg-[var(--bg)] py-6">
      <div className="w-[80%] px-6">
        ...
        <h1 className="font-['Open Sans'] text-[28px] font-extrabold text-center text-[var(--text)] mb-1">Join the gym</h1>
        <p className="text-[14px] text-[var(--text2)] text-center mb-8">Register as a society member</p>

        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="block text-[11px] font-semibold text-[var(--text2)] uppercase tracking-[0.06em] mb-2">Full name</label>
            <input
          ...
          <div>
            <label className="block text-[11px] font-semibold text-[var(--text2)] uppercase tracking-[0.06em] mb-2">Phone number</label>
            <input
              className="w-full bg-[var(--bg3)] border border-[var(--border)] rounded-[10px] px-4 py-3 text-[var(--text)] outline-none transition-all duration-200 font-['Open Sans'] placeholder:text-[var(--text3)] focus:border-[var(--accent)]"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-[var(--text2)] uppercase tracking-[0.06em] mb-2">Villa number</label>
              <input
                className="w-full bg-[var(--bg3)] border border-[var(--border)] rounded-[10px] px-4 py-3 text-[var(--text)] outline-none transition-all duration-200 font-['Open Sans'] placeholder:text-[var(--text3)] focus:border-[var(--accent)]"
                placeholder="12"
                value={form.flatNumber} onChange={set('flatNumber')} required
              />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-[var(--text2)] uppercase tracking-[0.06em] mb-2">Street / zone</label>
              <input
                className="w-full bg-[var(--bg3)] border border-[var(--border)] rounded-[10px] px-4 py-3 text-[var(--text)] outline-none transition-all duration-200 font-['Open Sans'] placeholder:text-[var(--text3)] focus:border-[var(--accent)]"
                placeholder="A"
                value={form.tower} onChange={set('tower')}
              />
            </div>
          </div>

          <div>
            <label className="block text-[11px] font-semibold text-[var(--text2)] uppercase tracking-[0.06em] mb-2">Password</label>
            <input
              className="w-full bg-[var(--bg3)] border border-[var(--border)] rounded-[10px] px-4 py-3 text-[var(--text)] outline-none transition-all duration-200 font-['Open Sans'] placeholder:text-[var(--text3)] focus:border-[var(--accent)]"
          ...
          >
            {loading ? <span className="h-5 w-5 rounded-full border-4 border-white border-t-transparent animate-spin" /> : 'Submit registration'}
          </button>
        </form>

        <p className="text-center text-[13px] text-[var(--text2)] mt-6">
          Already registered? <Link className="text-[var(--accent)] font-semibold" to="/login">Sign in</Link>
        </p>
      </div>
    </div>
  )
